  ...
  private generateProcessStepQuestions(processSteps: any[]): QuizQuestion[] {
    ...
    processSteps.forEach((step, index) => {
      if (step.responsible && step.activity) {
        ...
        // Get unique options and ensure we have at least 2 alternatives
        const uniqueOthers = [...new Set(otherResponsibles)];
        
        if (uniqueOthers.length >= 2) {
          // Take exactly 2 alternative options
          const alternatives = uniqueOthers.slice(0, 2);
          const options = [step.responsible, ...alternatives];
      ...
      if (step.inputs && step.inputs.length > 0) {
        ...
        // Get unique inputs and ensure we have at least 2 alternatives
        const uniqueOthers = [...new Set(otherInputs)];
        
        if (uniqueOthers.length >= 2) {
          const correctInput = step.inputs[0];
          // Take exactly 2 alternative options
          const alternatives = uniqueOthers.slice(0, 2);
          const options = [correctInput, ...alternatives];
          const shuffledOptions = this.shuffleArray(options);
    ...
    raciData.forEach((raci) => {
      if (raci.responsible && raci.task) {
        ...
        if (uniqueOthers.length >= 2) {
          // Take exactly 2 alternative options
          const alternatives = uniqueOthers.slice(0, 2);
          const options = [raci.responsible, ...alternatives];
          const shuffledOptions = this.shuffleArray(options);
      ...
      if (raci.accountable && raci.task) {
        ...
        // Get unique options and ensure we have at least 2 alternatives
        const uniqueOthers = [...new Set(otherAccountables)];
        
        if (uniqueOthers.length >= 2) {
          // Take exactly 2 alternative options
          const alternatives = uniqueOthers.slice(0, 2);
          const options = [raci.accountable, ...alternatives];
          const shuffledOptions = this.shuffleArray(options);
